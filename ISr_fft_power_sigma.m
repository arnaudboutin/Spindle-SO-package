...
% script to initialize the computation of the infraslow oscillation
% spectrum
...
%% variables 
freq_is=0.001:0.001:0.12; % infraslow frequency band
...
for num_subject=subject_to_process % loop over each subject
    ...
    for i_night=1:numel(file_name) % loop over each night
        ...
        try % Read Scoring file
            i_scoringFile = [ScoringDir 'hypno_S' subject_str '_night_' night_type '.mat']; % scoring file suffix 
            load (i_scoringFile)
        catch
        ...
        for ichan=1:numel(chan2process) % loop over each channel
            ...
            % process
            [spectrum_infraslow.(current_chan)(idx_subject,:),spectrum_infraslow_weighted.(current_chan)(idx_subject,:)]=ISr_compute_fft_sigma(Data,hypno_night,stage,method,freq_is,time_specific);
        end
...
% save
save([OutpuDir 'spectrum_infraslow_weighted_' night_type '_' stage '_' method idx_time '.mat'],'spectrum_infraslow_weighted','freq_is')
save([OutpuDir 'spectrum_infraslow_' night_type '_' stage '_' method idx_time '.mat'],'spectrum_infraslow','freq_is')
