function Info = ISr_SO_getInfoDefaults(Info, type, method)
...
switch type
    case 'SW'
        ...
        Info.Parameters.Filter_window=[0.5 4];
        ...
        Info.Parameters.Ref_InspectionPoint = 'ZC'; % 'MNP'/'ZC'
    ...
    case 'SS'
        ...
        % Filter Parameters
        Info.Parameters.Filter_Method = 'Chebyshev';      % 'Chebyshev'/'Buttersworth'
        Info.Parameters.Filter_band = [11, 16]; %11 17
        Info.Parameters.Filter_checkrange = 3;
        Info.Parameters.Filter_Window = 0.10; % length of smoothing window for root mean square of power
        Info.Parameters.Filter_order = 2;
        ...
        % Spindle Criteria       
%         Info.Parameters.Ref_AmplitudeRelative = 5;   %5    %7    % Standard deviations from mean negativity for onset / offset
%         Info.Parameters.Ref_AmplitudeRelative_Hard = 10;  %10      %12    % Standard deviations from mean negativity for peak max
        
        % Spindle Criteria       
        Info.Parameters.Ref_AmplitudeCriteria = 'relative'; % relative/absolute
        Info.Parameters.Ref_AmplitudeMetric = 'mean';  % mean / median
        ...
        Info.Parameters.Ref_NeighbourRatio = 3; % 2 % minimum tolerance for spindle/neighbour power ratio
       
        Info.Parameters.Ref_PowerGapMax = 0.15; %0.15      % Maximum gap between power above threshold (s)
        
        Info.Parameters.Ref_WaveLength = [0.3 2];      % Length criteria between zero crossings
    ...
    case 'SO_Alb'
        ...
        Info.Parameters.Amplitude_PTP=[75 500]; %microvolt
        Info.Parameters.Duration_negpeak=[0.3 1.5]; % duration of the negativ peak
        Info.Parameters.Duration_pospeak=[0.1 1];
        Info.Parameters.Bandpass_freq=[0.5 2];
        Info.Parameters.Duration_SO=[0.8 2];
        Info.Parameters.Threshold_method='quantile'; % std

    case 'SO_Sta'

        Info.Parameters.Bandpass_freq=[0.5 1.25]; % SO frequency
        Info.Parameters.min_SO_duration=0.8; % minimum duration, in sec
        Info.Parameters.max_SO_duration=2; % maximum duration, in sec
        Info.Parameters.amplitude_prctile=75; % percentile amplitude criteria
